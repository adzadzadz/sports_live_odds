...
<section id="nbaData" class="sloSportData row">
  <header class="col-md-8">
    <section class="row slo-filter-wrap">
      ...
      <div class="slo-dropdown col-md-6 slo-filter">
        <div class="slo-dropdown-toggle filter-label" href="#" role="button" id="nbaTypeDropdown">
          <span id="nbaTypeText" data-type="type">Type</span> <i class="fa fa-chevron-down"></i>
        </div>

        <div class="slo-dropdown-menu slo-hidden">
          <div class="oddsNba slo-dropdown-item" data-type="type" data-value="PointSpread" >SPREAD</div>
          <div class="oddsNba slo-dropdown-item" data-type="type" data-value="OverUnder" >TOTAL</div>
          <div class="oddsNba slo-dropdown-item" data-type="type" data-value="MoneyLine" >MONEYLINE</div>
        </div>
      </div>
    </section>
  </header>
  <section id="nbaContentContainer" class="col-md-12"></section>
</section>

<script async>
  class NBA extends SLO {
    build () {
      // #1 Date Setup
      jQuery("#nbaData #dateDisplay").text(this.getClosestDateFromList(new Date(sloCurrentStartDate)));
      let selectedDate = jQuery("#nbaData #dateDisplay").text();

      // #2 The actual request
      this.fetchData(
        "https://api.sportsdata.io/v3/nba/odds/json/GameOddsByDate/" + selectedDate + "?key=<?= $this->config['apiKeys']['nba']['liveOdds'] ?>",
        this.type
      );

      // #3 Book Type Setup
      jQuery("#nbaData #nbaTypeText").html(this.type.toUpperCase());
      jQuery("#nbaData #nbaTypeText").data("type", this.type);
  ...
  (function() {
    jQuery(document).ready(() => {
      let nba = new NBA();
      nba.sport = 'nba';
      nba.sportsBooks = ['Pinnacle', 'WestgateSuperbookNV', 'DraftKings', 'FanDuel', 'SugarHousePA'];
      nba.build();
      ...
      jQuery(".dateChanger").on('click', function(e) {
        ...
        if (dateElem.data('type') == "prev") {
          newDate = nba.getClosestDateFromList(selected, - 1);
        }
        ...
        nba.fetchData(
          "https://api.sportsdata.io/v3/nba/odds/json/GameOddsByDate/" + newDate + "?key=<?= $this->config['apiKeys']['nba']['liveOdds'] ?>",
          jQuery("#nbaData #nbaTypeText").data("type")
        );
      });

      // Change Book Type
      jQuery(".oddsNba.slo-dropdown-item").on("click", function(e) {
        if ( jQuery(this).data("type") == "type" ) {
          nba.type = jQuery(this).data("value");
          text = jQuery(this).html();
          jQuery("#nbaData #nbaTypeText").html(text.toUpperCase());
          jQuery("#nbaData #nbaTypeText").data("type", nba.type);
        }
        if (nba.resultData)
          nba.setSloOddsView(nba.resultData, nba.sport, nba.type);
      });
    });
  })();
</script>